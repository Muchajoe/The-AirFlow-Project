...
That includes the Rotary example and adds a WebUi. After flashing this you should be able to connect to a wlan called **"The Airflow Project"** with a password of **"TheAirFlowSetup"**. It will also read out the RPM and it will also try to readout the Wattage (that only works if you have bridged the J1 Jumper on the PCB). If you want to use a rotary encoder:
